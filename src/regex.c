...
extern double max_mot_offset, mot_offset_factor, mot_factor;
...
void regex_parse(char *mot_regex)
{
  ...
  if ((mot_pos = (short **)malloc(seqnum * sizeof(short *))) == NULL)
  {
  ...
  for (i = 0; i < seqnum; i++)
    ...
    if ((mot_pos[i] = (short *)malloc((seqlen[i] + 2) * sizeof(short))) == NULL)
    {
      printf(" problems with memory allocation");
  ...
  */
  ...
  for (p = 0; p < mot_len; p++)
  {
    ...
    mot_char[p] = (char *)malloc(char_num[p] * sizeof(char));
  }
