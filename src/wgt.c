...
extern double **min_weight, threshold, av_sim_score_pep, av_sim_score_nuc;
...
void rel_wgt_calc(int l1, int l2, double **rel_wgt)
{
  int l, m, mss;
  double ent, factor, l1f, l2f, **wgt, av_sim_score;
...
void wgt_prnt_prot(int seqlen_l1, int seqlen_l2)
{
  int i, j;
...
void wgt_prnt_dna(int seqlen_l1, int seqlen_l2)
{
  int i, j;
...
void wgt_prnt_trans(int seqlen_l1, int seqlen_l2)
{
  int i, j;
...
void wgt_prnt(int seqlen_l1, int seqlen_l2)
{
  if (wgt_type == 0)
    wgt_prnt_prot(seqlen_l1, seqlen_l2);

  if (wgt_type % 2)
    wgt_prnt_dna(seqlen_l1, seqlen_l2);

  if (wgt_type > 1)
    wgt_prnt_trans(seqlen_l1, seqlen_l2);
}
...
void mem_alloc()
{
  ...
  if (wgt_type == 0)
  {
    if ((tp400_prot = (double **)calloc((max_dia + 1), sizeof(double *))) == NULL)
    {
      printf(" problems with memory allocation for `tp400_prot' !  \n \n");
      exit(1);
    }

    if ((wgt_prot = (double **)calloc((max_dia + 1), sizeof(double *))) == NULL)
    {
  ...
  if (wgt_type % 2)
  {
    if ((tp400_dna = (double **)calloc((max_dia + 1), sizeof(double *))) == NULL)
    {
      printf(" problems with memory allocation for `tp400_dna' !  \n \n");
      exit(1);
    }

    if ((wgt_dna = (double **)calloc((max_dia + 1), sizeof(double *))) == NULL)
    {
      printf(" problems with memory allocation for `weights' !  \n \n");
      exit(1);
    }
  ...
  if (wgt_type > 1)
  {
    if ((tp400_trans = (double **)calloc((max_dia + 1), sizeof(double *))) == NULL)
    {
      printf(" problems with memory allocation for `tp400_trans' !  \n \n");
      exit(1);
    }

    if ((wgt_trans = (double **)calloc((max_dia + 1), sizeof(double *))) == NULL)
    {
      printf(" problems with memory allocation for `weights' !  \n \n");
      exit(1);
    }
  ...
  for (i = 1; i <= max_dia; i++)
  {
    ...
    {
      if ((tp400_prot[i] =
               (double *)calloc(((i + 1) * max_sim_score), sizeof(double))) == NULL)
      {
        printf(" problems with memory allocation for `tp400_prot' !  \n \n");
        exit(1);
      }
    ...
    {
      if ((tp400_dna[i] =
               (double *)calloc(((i + 1)), sizeof(double))) == NULL)
      {
        printf(" problems with memory allocation for `tp400_dna' !  \n \n");
        exit(1);
      }
